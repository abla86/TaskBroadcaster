...
app.MapGet("/api/tasks", (ITaskStore store) => Results.Ok(store.GetAll()));
app.MapGet("/api/tasks/{id:guid}", (Guid id, ITaskStore store) =>
    store.Get(id) is { } item ? Results.Ok(item) : Results.NotFound());

app.MapPost("/api/tasks", async (TaskRequest req, ITaskStore store, IHubContext<TaskHub> hub) =>
{
...
public record WorkItem(Guid Id, string Title, string Status, DateTime CreatedAt);
...
public class InMemoryTaskStore : ITaskStore
{
    ...
    public IEnumerable<WorkItem> GetAll() => _items.Values.OrderByDescending(x => x.CreatedAt);
    public WorkItem? Get(Guid id) => _items.TryGetValue(id, out var item) ? item : null;

    public WorkItem Add(string title)
    {
        var item = new WorkItem(Guid.NewGuid(), title, "Pending", DateTime.UtcNow);
        _items[item.Id] = item;
        return item;
    }
    ...
    ILogger<TaskWorkerService> logger) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            var pending = store.GetAll().FirstOrDefault(t => t.Status == "Pending");
            if (pending is null) continue;

            var updated = store.UpdateStatus(pending.Id, "Completed");
            if (updated is not null)
            {
                logger.LogInformation("Jobb ferdigstilt: {Title} ({Id})", updated.Title, updated.Id);
                await hub.Clients.All.SendAsync("TaskUpdated", updated, stoppingToken);
            }
        }
    }
}